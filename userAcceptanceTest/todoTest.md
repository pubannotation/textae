...
1. コンフィグレーション保存ダイアログでは、コンフィグレーション変更前後の diff を表示します
2. diff を取得するために、変更前のコンフィグレーション情報を保存しています
3. URL からコンフィグレーションファイルを読み込んだ時に、変更前のコンフィグレーション情報を読み込んだコンフィグレーションに更新していませんでした
4. diff に表示するコンフィグレーション情報が、読み込み前のコンフィグレーションになっていました
5. 発生バージョン不明
6. 対応バージョン未定
...
4. `q`キーを押してパレットを開く
5. `Uplaod`ボタンをクリック
6. `Configuration differences`欄の`attribute types`の 1 番目の`pred`の変更前の値が`denote`でないこと
