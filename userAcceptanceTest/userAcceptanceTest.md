...
1. 重複アトリビュートのアトリビュート定義を変更すると、同一の Entity を二回レダリングします。
2. このときの一回目のレンダリングでは片方のアトリビュートの更新が終わっていないため、定義が存在しないアトリビュートの表示名を取得しようとします
3. 6.4.91 で、定義が存在しないアトリビュートの表示名を取得をガード条件から、アサーションに変えたため、エラーが起きるようになりました
...
6. `Edit this predicate.`ボタンをクリック
7. `label`と`color`の値を変更する
8. `OK`ボタンを押す
9. パレット上の`label`と`color`の値が、変更後の値に変わること
