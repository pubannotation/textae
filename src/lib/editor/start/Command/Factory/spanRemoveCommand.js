...
export default function(editor, annotationData, selectionModel, id) {
  const removeEntity = _.flatten(annotationData.span.get(id).getTypes().map((type) => type.entities.map((entity) => entityAndAssociatesRemoveCommand(editor, annotationData, selectionModel, entity.id))))
