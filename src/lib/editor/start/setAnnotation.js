...
export default function(spanConfig, typeDefinition, annotationData, annotation, config) {
  const ret = setConfigInAnnotation(spanConfig, typeDefinition, annotation)

  if (ret === 'no config') {
    setConfigFromServer(spanConfig, typeDefinition, annotationData, config, annotation)
  } else {
