const allModificationClasses =
  'textae-editor__negation textae-editor__speculation'

export default function(annotationData) {
  return {
    getClasses: (objectId) => getClasses(annotationData, objectId),
    update: (domElement, objectId) =>
      update(annotationData, domElement, objectId)
  }
}
...
function update(annotationData, domElement, objectId) {
  ...
  getClasses(annotationData, objectId).forEach((c) =>
    domElement.classList.add(c)
  )
}
