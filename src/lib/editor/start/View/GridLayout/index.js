...
export default function(editor, annotationData, typeContainer) {
  const domPositionCaChe = new DomPositionCache(editor, annotationData.entity),
    arrangePositionAll = (typeGapValue) => {
      domPositionCaChe.reset()
      return Promise.all(genArrangeAllGridPositionPromises(domPositionCaChe, typeContainer, typeGapValue, annotationData))
    }
...
function genArrangeAllGridPositionPromises(domPositionCaChe, typeContainer, typeGapValue, annotationData) {
  return annotationData.span.all()
    ...
    .map((span) => arrangeGridPositionPromise(domPositionCaChe, typeContainer, typeGapValue, annotationData, span))
}
